...
<body class="login-layout light-login" style="background: #DFE0E2" onkeyup="if(event.keyCode == 13){login();};">
					...
					<div class="login-container">
						<div class="center">
							<h1>
								...
								<span class="block white" id="id-text2">超管登录</span>
							</h1>
											...
											<fieldset>
												<label class="block clearfix">
													<span class="block input-icon input-icon-right">
														<input id="account" type="text" autofocus="" name="name" class="form-control" placeholder="请输入帐号" value="root" />
														<i class="ace-icon fa fa-user"></i>
													</span>
												</label>
												<label class="block clearfix">
													<span class="block input-icon input-icon-right">
														<input id="password" type="password" name="password" class="form-control" placeholder="请输入密码" value="root" />
												...
												<label class="block clearfix">
													<div class="row">
														<div class="col-xs-7">
															<span class="block input-icon input-icon-right">
																<input id="captcha" name="captcha" type="text" class="form-control" placeholder="请输入四位验证码">
															</span>
														</div>
														<div class="col-xs-5">
	...
	<script type="text/javascript">
		...
		function alert_msg(msg){
			var html_code = '<div class="alert alert-block alert-danger"><button type="button" class="close" data-dismiss="alert">×</button>' + msg + '</div>';
			$('#alert').html(html_code);
			return false;
		}
		...
		function check_data(){
			...
			// 2. 做验证(非空)
			if (account == ''){
				if (!alert_msg('用户名不能为空')){return false;}
			}else if (password == ''){
				if (!alert_msg('密码不能为空')){return false;}
			}else if (captcha == ''){
				if (!alert_msg('验证码不能为空')){return false;}
			}
			return true;
		}
		// 登录的提交方法
		function login() {
			...
			// 提交到后端
			$.post('/ajax/login-super.php',$('form').serialize(),function(res){
				if (res.tag > 0) {
					$('#login').button('reset');
					$('#alert').html(res.html);
					return false;
					...
					setTimeout(function(){
						$('#login').button('reset');
						$('#login').text('登录成功');return;
						window.location.href="/super/index.php";
					},1000);
				}
			},'json');
