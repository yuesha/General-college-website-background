...
<body>
								...
								<div class="tab-content">
									...
									<div id="edit" class="tab-pane active">
										...
										<form class="form-horizontal" id="sample-form">
											...
											<div class="form-group">
												<label for="up_id" class="col-xs-12 col-sm-3 control-label no-padding-right">隶属内容分类</label>
												...
												<div class="col-xs-12 col-sm-5 inline help-block">
													<span class="block input-icon input-icon-right">
														<select name="up_id" id="up_id" class="width-100">
															...
															<?php
																$cont_types = $mysql -> findAll("cont_type","del_tag = 0 && show_tag = 0");
																foreach ($cont_types as $k => $v) {
															?>
											...
											<div class="form-group">
												<label for="type" class="col-xs-12 col-sm-3 control-label no-padding-right">信息分类类型</label>

												<div class="col-xs-12 col-sm-5 inline help-block">
													<span class="block input-icon input-icon-right">
														<select name="type" id="type" class="width-100">
															<option value="0" <?php echo $row['type']==0?'selected':'';?>>有下级分类</option>
															<option value="1" <?php echo $row['type']==1?'selected':'';?>>新闻列表</option>
															<option value="2" <?php echo $row['type']==2?'selected':'';?>>图文列表</option>
															<option value="3" <?php echo $row['type']==3?'selected':'';?>>直接内容</option>
															<option value="4" <?php echo $row['type']==4?'selected':'';?>>链接地址</option>
														</select>
														<i class="icon-leaf"></i>
													</span>
												</div>
	...
	<script type="text/javascript">
		function save() {
			$('#loading').modal('show');
			$.post('/ajax/type_manage-edit.php',$('form').serialize(),function(res){
				if (res.tag > 0) {
					$('#loading').modal('hide');
					$('#alert').html(res.html);
					return false;
					...
					setTimeout(function(){
						$('#loading').modal('hide');
						$('#login').text('成功');
						window.location.href="/super/type_manage-edit.php?id=<?php echo $row['id'];?>";
					},1000);
				}
			},'json');
			return false;
