...
<body>
									...
									<div id="dtr" class="tab-pane active">
										...
										<div class="table-responsive">
											<table id="sample-table-1" class="table table-striped table-bordered table-hover">
												...
												<tbody>
													<?php
													$rows = $mysql -> findAll("menus","del_tag=1 ORDER BY show_order asc");
													foreach ($rows as $k => $v) {
														// 查询父级菜单的记录
														$up_row = array();
														if ($v['up_id']) {
															$up_row = $mysql -> find('menus',"id={$v['up_id']}");
														}
													?>
													<tr>
														<td class="center">
	...
	<script type="text/javascript">
		function reset(id) {
			...
			if (bool) {
				$('#loading').modal('show');
				$.post('/ajax/menu_manage-reset.php',{'id':id},function(res){
					if (res.tag > 0) {
						$('#loading').modal('hide');
						$('#alert').html(res.html);
						return false;
						...
						setTimeout(function(){
							$('#loading').modal('hide');
							$('#login').text('成功');
							window.location.href="/super/menu_manage-dtr.php";
						},1000);
					}
				},'json');
			}
