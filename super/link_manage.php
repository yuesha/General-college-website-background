...
<body>
									...
									<div id="lists" class="tab-pane in active">
										...
										<div class="table-responsive">
											<table id="sample-table-1" class="table table-striped table-bordered table-hover">
												...
												<tbody>
													<?php
													$rows = $mysql -> findAll("links","del_tag=0 ORDER BY created_at desc");
													foreach ($rows as $k => $v) {
													?>
													<tr>
														<td class="center">
														...
														<td><?php echo $v['tag']==0?'校内链接':'校外链接';?></td>
														<td class="hidden-480"><?php echo $v['title'];?></td>
														<td class="hidden-480"><?php echo $v['src'];?></td>
														<td class="hidden-480">
	...
	<script type="text/javascript">
		function edit(id) {
			window.location.href="link_manage-edit.php?id=" + id;
		}
		...
		function del(id) {
			...
			if (bool) {
				$('#loading').modal('show');
				$.post('/ajax/link_manage-del.php',{'id':id},function(res){
					if (res.tag > 0) {
						$('#loading').modal('hide');
						$('#alert').html(res.html);
						return false;
						...
						setTimeout(function(){
							$('#loading').modal('hide');
							$('#login').text('成功');
							window.location.href="/super/link_manage.php";
						},1000);
					}
				},'json');
			}
