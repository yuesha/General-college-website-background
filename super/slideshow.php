...
<body>
									...
									<div id="lists" class="tab-pane in active">
										...
										<div class="table-responsive">
											<table id="sample-table-1" class="table table-striped table-bordered table-hover">
												...
												<tbody>
													<?php
													$rows = $mysql -> findAll("pics","del_tag=0 && type=1 ORDER BY created_at desc");
													foreach ($rows as $k => $v) {
														?>
														<tr>
															<td class="center">
	...
	<script type="text/javascript">
		function edit(id) {
			window.location.href="slideshow-edit.php?id=" + id;
		}
		...
		function del(id) {
			...
			if (bool) {
				$('#loading').modal('show');
				$.post('/ajax/slideshow-del.php',{'id':id},function(res){
					if (res.tag > 0) {
						$('#loading').modal('hide');
						$('#alert').html(res.html);
						return false;
						...
						setTimeout(function(){
							$('#loading').modal('hide');
							$('#login').text('成功');
							window.location.href="/super/slideshow.php";
						},1000);
					}
				},'json');
			}
