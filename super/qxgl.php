...
<body>
									...
									<div id="lists" class="tab-pane in active">
										...
										<div class="table-responsive">
											<table id="sample-table-1" class="table table-striped table-bordered table-hover">
												...
												<tbody>
													<?php
													$rows = $mysql -> findAll("manage","del_tag=0 ORDER BY created_at desc");
													foreach ($rows as $k => $v) {
														$is_empower = $mysql -> find("authority","manage_id={$v['id']}");
													?>
													<tr>
														<td class="center">
															<?php echo $k+1;?>
	...
	<script type="text/javascript">
		function edit(id) {
			window.location.href="qxgl-edit.php?id=" + id;
		}
		...
		function del(id) {
			...
			if (bool) {
				$('#loading').modal('show');
				$.post('/ajax/qxgl-del.php',{'id':id},function(res){
					if (res.tag > 0) {
						$('#loading').modal('hide');
						$('#alert').html(res.html);
						return false;
						...
						setTimeout(function(){
							$('#loading').modal('hide');
							$('#login').text('成功');
							window.location.href="/super/qxgl-edit.php?id=<?php echo $id;?>";
						},1000);
					}
				},'json');
			}
