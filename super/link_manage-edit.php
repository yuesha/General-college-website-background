...
<body>
									...
									<div id="edit" class="tab-pane active">
										...
										<form class="form-horizontal" id="sample-form">
												...
												<div class="col-xs-12 col-sm-5 inline help-block">
													<span class="block input-icon input-icon-right">
														<select name="type" id="type" class="width-100">
															<option value="0" <?php echo $row['tag']==0?'selected':'';?>>校内链接</option>
															<option value="1" <?php echo $row['tag']==1?'selected':'';?>>校外链接</option>
														</select>
														<i class="icon-leaf"></i>
													</span>
												</div>
											...
											<div class="form-group">
												...
												<div class="col-xs-12 col-sm-5 inline help-block">
													<span class="block input-icon input-icon-right">
														<input type="text" name="name" id="name" class="width-100" value="<?php echo $row['title'];?>">
														<i class="icon-leaf"></i>
													</span>
												</div>
												<div class="help-block col-xs-12 col-sm-reset inline red">
											...
											<div class="form-group">
												...
												<div class="col-xs-12 col-sm-5 inline help-block">
													<span class="block input-icon input-icon-right">
														<input type="text" name="src" id="src" class="width-100" value="<?php echo $row['src'];?>">
														<i class="icon-leaf"></i>
													</span>
												</div>
												<div class="help-block col-xs-12 col-sm-reset inline red">
	...
	<script type="text/javascript">
		function save() {
			$('#loading').modal('show');
			$.post('/ajax/link_manage-edit.php',$('form').serialize(),function(res){
				if (res.tag > 0) {
					$('#loading').modal('hide');
					$('#alert').html(res.html);
					return false;
					...
					setTimeout(function(){
						$('#loading').modal('hide');
						$('#login').text('成功');
						window.location.href="/super/link_manage-edit.php?id=<?php echo $id;?>";
					},1000);
				}
			},'json');
			return false;
