...
class mysql {
    ...
    private $dbName = 'gwb';
    /* 用户名 */
    private $UserName = 'gwb';
    /* 连接密码 */
    private $Password = 'gwb123';
    ...
    public function insert3($table,$array){
        $keys = join(",",array_keys($array));
        $vals = "'".join("','",array_values($array))."'";
        $sql = "insert into {$table}({$keys}) values({$vals})";
        // echo $sql;
    ...
    */
    ...
    // $action    动作
    // $caozuo    操作级别
    public function rizhi($tableName,$name ='',$action ='',$sql ='',$caozuo=''){
        $time = date("Y-m-d H:i:s"); //获取当前时间
        $ip = $_SERVER["REMOTE_ADDR"]; //获取ip地址
        if(empty($sql)){
            $sql = $this->getLastSql();   //获取最后一次操作的sql语句
        }
        switch ($caozuo) {
            case '1':
                $caozuo = "数据插入";
        ...
        $sql = "INSERT INTO ".$tableName. "(user,created_at,ip,action,sql_cont,db_level) VALUES (
        '{$name}','{$time}','{$ip}','{$action}','{$sql}','{$caozuo}')";
        $query = mysql_query($sql);
    }
}
// 日志操作函数 2018-2-5  宋志永 更新
