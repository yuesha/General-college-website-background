...
// 实例化类库
$mysql = new mysql();
// 修改banner头图片（横幅图片）并记录日志
$data['type'] = 2;// banner头图片（横幅图片）
...
$info['action'] = "修改banner头图片（横幅图片）:".@basename($filename)."成功";
$info['sql_cont'] = str_replace('"', "'", $mysql -> getLastSql());
$mysql -> insert("super_log",$info);

exit_msg("修改banner头图片（横幅图片）:".@basename($filename)."成功","success",0);
?>